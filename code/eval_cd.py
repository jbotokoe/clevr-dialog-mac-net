...
def evaluate(args):
    ...
    for idx, data in enumerate(t, 0):
        ...
        preds = preds.view(10, -1).transpose(0,1).contiguous().view(-1).cpu().numpy()
        answer = answer.view(10, -1).transpose(0,1).contiguous().view(-1).cpu().numpy()
